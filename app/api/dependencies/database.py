...
#  Last Modified on 2024/01/31 09:06:47
#  #
...
def _get_db_connection(request: Request) -> Connection:
    return request.app.state.sqlite


def get_repository(repo_type: Type[BaseRepository]) -> Callable[[Connection], BaseRepository]:
    def _get_repo(conn: Connection = Depends(_get_db_connection), ) -> BaseRepository:
